#!/usr/bin/env python3
"""
API test for LTX-2.3 22B Dev FP8 text-to-video on ROCm R9700.
25 steps, CFG=3.0, 768x512 121 frames (~5s @ 24fps). Full non-distilled model.
...
OUTPUT_PREFIX = "ROCM_R9700_LTX23_Dev"
...
def main():
    print(f"[LTX23-Dev] Queuing prompt to {SERVER}")
    print("[LTX23-Dev] 22B Dev model — 25 steps, 121 frames. Expect 20-40 min on first run.")
    t0 = time.time()
    prompt_id = queue_prompt(WORKFLOW)
    print(f"[LTX23-Dev] Prompt ID: {prompt_id}")
    ...
    if videos:
        print(f"[LTX23-Dev] SUCCESS in {elapsed:.1f}s — {len(videos)} video(s):")
        for v in videos:
            print(f"  {v.get('filename', v)}")
    else:
        print(f"[LTX23-Dev] FAILED after {elapsed:.1f}s — no videos in output")
